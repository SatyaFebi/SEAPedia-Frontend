...
const voucherForm = ref({ code: '', type: 'percentage', value: 10, minPurchase: 50000, maxDiscount: 20000, description: '' })
...
function handleCreateVoucher() {
  ...
  voucherForm.value = { code: '', type: 'percentage', value: 10, minPurchase: 50000, maxDiscount: 20000, description: '' }
  setTimeout(() => { promoAlertMessage.value = '' }, 4000)
}
...
function handleSimulateNextDay() {
  ...
  setTimeout(() => { slaTransitionAlert.value = false }, 4000)
}
  ...
  <div class="space-y-6 text-[#374151]">
    ...
    <div class="card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-[#6B7280] mt-0.5">Simulasi hari, kelola voucher, dan monitor seluruh transaksi sistem.</p>
      </div>
    ...
    <!-- SLA alert -->
    <div v-if="slaTransitionAlert" class="px-4 py-3 bg-accent-rose-50 border border-accent-rose-100 rounded-xl text-sm text-accent-rose-700">
      <strong>Simulasi selesai!</strong> Order melewati SLA otomatis dikembalikan dan paket di jalan ditandai terkirim.
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="stat-card">
        ...
        <p class="stat-value text-primary-600 text-xl">Rp{{ totalTransactionVolume.toLocaleString('id-ID') }}</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="card p-6 space-y-5">
        <h3 class="font-display font-semibold text-[#0D1117] pb-4 border-b border-[#E5E8EC]">Generator Voucher</h3>

        <div v-if="promoAlertMessage" class="px-3 py-2.5 bg-primary-50 border border-primary-100 rounded-lg text-sm text-primary-700">
          {{ promoAlertMessage }}
        </div>
        ...
        <form @submit.prevent="handleCreateVoucher" class="space-y-4">
          <div>
            ...
            <input type="text" v-model="voucherForm.code" placeholder="PROMOJUNI" class="input uppercase" required />
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" v-model.number="voucherForm.maxDiscount" :disabled="voucherForm.type === 'delivery'" class="input disabled:opacity-40" />
            </div>
          </div>
          <div>
            <label class="input-label">Deskripsi (Opsional)</label>
            <input type="text" v-model="voucherForm.description" class="input" placeholder="Promo spesial..." />
          </div>
      ...
      <div class="card p-6 lg:col-span-2 space-y-5">
        <h3 class="font-display font-semibold text-[#0D1117] pb-4 border-b border-[#E5E8EC]">Monitor Pengiriman Global</h3>
          ...
          <table class="w-full text-sm border-collapse">
            ...
            <tbody class="divide-y divide-[#E5E8EC]">
              <tr v-if="orders.length === 0">
                <td colspan="5" class="py-8 text-center text-sm text-[#9CA3AF]">Tidak ada order.</td>
              </tr>
              <tr v-for="order in orders" :key="order.id" class="hover:bg-[#F4F6F8] transition-colors">
                ...
                <td class="py-3 px-3">
                  <span class="text-xs font-mono" :class="order.days_in_current_status >= 1 ? 'text-accent-rose-600 font-semibold' : 'text-[#9CA3AF]'">
