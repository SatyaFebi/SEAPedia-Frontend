...
<template>
  <div class="min-h-screen flex flex-col md:flex-row bg-[#FAFAFA] font-sans text-[#374151]">
    ...
    <aside class="w-full md:w-64 shrink-0 bg-white border-r border-[#E5E8EC] flex flex-col">
      ...
      <div class="px-4 py-4 border-b border-[#E5E8EC]">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-full bg-primary-600 flex items-center justify-center font-display font-bold text-white text-sm shrink-0">
        ...
        <!-- Wallet (Buyer only) -->
        <div v-if="activeRole === 'Buyer' && user" class="mt-3 px-3 py-2.5 rounded-lg bg-[#F4F6F8] border border-[#E5E8EC] flex items-center justify-between">
      ...
      <nav class="flex-1 p-3 space-y-0.5">
        ...
        <template v-if="activeRole && roleConfig[activeRole]">
          <router-link
            ...
            :class="route.name === roleConfig[activeRole].route
      ...
      <div class="p-3 border-t border-[#E5E8EC] space-y-2">
        ...
        <div v-if="hasMultipleRoles" class="relative group">
          <button class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs text-[#374151] hover:bg-[#F4F6F8] transition-colors border border-[#E5E8EC] bg-white">
            <span class="flex items-center gap-2">🔄 Ganti Peran</span>
            <span class="text-[#9CA3AF]">▾</span>
          </button>
          <div class="absolute bottom-full left-0 right-0 mb-1.5 hidden group-hover:block bg-white border border-[#E5E8EC] rounded-xl shadow-lg z-20 overflow-hidden p-1">
            <button
        ...
        <button
          @click="handleLogout"
          class="btn-danger w-full text-xs py-2"
        >
          🚪 Keluar Sesi
        </button>
      </div>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <!-- Topbar -->
      <header class="h-16 bg-white border-b border-[#E5E8EC] px-6 flex items-center justify-between shrink-0">
        ...
        <div class="flex items-center gap-3 ml-auto">
          <!-- SLA Day chip -->
          <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#F4F6F8] border border-[#E5E8EC] text-xs text-[#374151]">
            <span class="w-1.5 h-1.5 rounded-full bg-primary-500 animate-pulse"></span>
            <span>Hari <strong class="font-semibold">{{ simulatedDay }}</strong></span>
          </div>
    ...
    <div v-if="isProfileModalOpen && user" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0D1117]/40 backdrop-blur-sm">
      <div class="w-full max-w-sm bg-white rounded-2xl border border-[#E5E8EC] p-6 shadow-xl space-y-4">
        <div class="flex items-start justify-between">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              <h3 class="font-display font-bold text-[#0D1117] text-sm">Profil &amp; Finansial Akun</h3>
              <p class="text-[10px] text-[#9CA3AF]">Username: @{{ user.username }}</p>
            </div>
          </div>
          <button @click="isProfileModalOpen = false" class="w-6 h-6 flex items-center justify-center rounded-lg bg-[#F4F6F8] text-[#6B7280] hover:text-[#0D1117] hover:bg-[#E5E8EC] transition-colors text-[10px] cursor-pointer shrink-0">
            ✕
          </button>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div class="bg-[#F4F6F8] rounded-xl p-3 space-y-1.5 text-xs">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-semibold text-[#374151] truncate max-w-[180px]">{{ user.email }}</span>
            </div>
          </div>
            ...
            <div class="flex flex-wrap gap-1.5">
              <span
                ...
                :class="user.roles.includes(role)
                  ? (activeRole === role ? 'bg-primary-600 text-white font-semibold' : 'bg-primary-50 text-primary-700 border border-primary-100')
          ...
          <div class="space-y-1.5">
            ...
            <div class="space-y-1">
              <!-- Buyer balance -->
              <div class="flex items-center justify-between p-2 rounded-lg bg-[#F4F6F8] border border-[#E5E8EC] text-xs">
                <span class="flex items-center gap-1">🛒 Buyer <span class="text-[#9CA3AF]">(Wallet)</span></span>
                <span class="font-semibold text-[#0D1117]">{{ user.roles.includes('Buyer') ? 'Rp' + user.walletBalance.toLocaleString('id-ID') : '-' }}</span>
              </div>
              <!-- Seller balance -->
              <div class="flex items-center justify-between p-2 rounded-lg bg-[#F4F6F8] border border-[#E5E8EC] text-xs">
                <span class="flex items-center gap-1">🏪 Seller <span class="text-[#9CA3AF]">(Income)</span></span>
                <span class="font-semibold text-[#0D1117]">{{ user.roles.includes('Seller') ? 'Rp' + (user.sellerIncome || 0).toLocaleString('id-ID') : '-' }}</span>
              </div>
              <!-- Driver balance -->
              <div class="flex items-center justify-between p-2 rounded-lg bg-[#F4F6F8] border border-[#E5E8EC] text-xs">
                <span class="flex items-center gap-1">🛵 Driver <span class="text-[#9CA3AF]">(Earnings)</span></span>
                <span class="font-semibold text-[#0D1117]">{{ user.roles.includes('Driver') ? 'Rp' + (user.driverEarnings || 0).toLocaleString('id-ID') : '-' }}</span>
              </div>
            </div>
          </div>
        </div>

        <button @click="isProfileModalOpen = false" class="btn-primary w-full justify-center text-xs py-2">Tutup</button>
      </div>
    </div>
